...
Directorio en el que se encuentran los archivos de log

.PARAMETER DirectorioZip
Directorio en el que se generarán los archivos comprimidos de los clientes

.PARAMETER Empresa
Nombre de la empresa a procesar. Es un parametro opcional.

.EJEMPLO DE EJECUCION
./Ejercicio4.sh  -Directorio <path_directorio_Log> -DirectorioZip <path_directorio_Zip> "
...
if( $cantPar -ne 3 )
{
	...
	$files=Get-ChildItem -Path $Directorio -Filter "*log" | Select-Object -Property Name | Where-Object { $_.Name -match '.+-[0-9]+.log' } | Sort-Object -Property Name, LastWriteTime -Descending
	foreach($file in $files.Name)
	{	
		$pos = $file.IndexOf("-") 
		$EMPRESA = $file.Substring(0, $pos)
		...
		if( $EMPRESA -eq $EMPRESA_INICIAL )
		{
			...
			$pos = $file.IndexOf("-") 
			$EMPRESA_INICIAL = $file.Substring(0, $pos)
		}
	}
}else{
	$VERSION=1
